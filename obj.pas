...
uses
  Classes, sysutils, _meshify_simplify_quadric;
procedure LoadObj(const FileName: string; var faces: TFaces;
...
procedure LoadObj(const FileName: string; var faces: TFaces;
  var vertices: TVertices); // ; var vertexRGBA: TVertexRGBA);
...
begin
  ...
  setlength(vertices, (fsz div 70) + kBlockSize);
  // guess number of faces based on filesize to reduce reallocation frequencey
  setlength(faces, (fsz div 35) + kBlockSize);
  // guess number of vertices based on filesize to reduce reallocation frequencey
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        faces[num_f].Y := strtointDef(strlst[j + 1], 0) - 1;
        // -1 since "A valid vertex index starts from 1"
        faces[num_f].Z := strtointDef(strlst[j + 2], 0) - 1;
        // -1 since "A valid vertex index starts from 1"
        inc(num_f);
      end;
    end;
    if (strlst.count > 3) and ((strlst[0]) = 'v') then
    begin
