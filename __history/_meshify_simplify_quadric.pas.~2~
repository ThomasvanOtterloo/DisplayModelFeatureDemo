unit _meshify_simplify_quadric;
...
Type
  TPoint3f = packed record
    ...
    Y: Single;
    Z: Single end;
    TPoint3i = packed record X: longint;
    Y: longint;
    Z: longint;
  end;

  TFaces = array of TPoint3i;
  // n.b. Triangle indexed from zero, so face composed of first three vertices is [0,1,2]
  TVertices = array of TPoint3f;

procedure simplify_mesh(var faces: TFaces; var verts: TVertices;
  target_count: integer; agressiveness: double = 7; resize: boolean = true);
...
function symMat(c: TFloat): TSymetricMatrix; {$IFDEF FPC}inline;
{$ENDIF} overload;
var
  i: integer;
begin
...
function symMat(a, b, c, d: TFloat): TSymetricMatrix; {$IFDEF FPC}inline;
{$ENDIF} overload;
begin
  result[0] := a * a;
  result[1] := a * b;
...
function symMat(m11, m12, m13, m14, m22, m23, m24, m33, m34, m44: TFloat)
  : TSymetricMatrix; {$IFDEF FPC}inline; {$ENDIF} overload;
begin
  result[0] := m11;
  result[1] := m12;
  result[2] := m13;
...
function symMatAdd(n, m: TSymetricMatrix): TSymetricMatrix; {$IFDEF FPC}inline;
{$ENDIF}
begin
  result := symMat(n[0] + m[0], n[1] + m[1], n[2] + m[2], n[3] + m[3],
    n[4] + m[4], n[5] + m[5], n[6] + m[6], n[7] + m[7], n[8] + m[8],
    n[9] + m[9]);
end; // symMatAdd()

function symMatDet(m: TSymetricMatrix; a11, a12, a13, a21, a22, a23, a31, a32,
  a33: integer): TFloat; {$IFDEF FPC}inline; {$ENDIF}
begin
  result := m[a11] * m[a22] * m[a33] + m[a13] * m[a21] * m[a32] + m[a12] *
    m[a23] * m[a31] - m[a13] * m[a22] * m[a31] - m[a11] * m[a23] * m[a32] -
    m[a12] * m[a21] * m[a33];
end; // symMatDet()
...
begin
  result := q[0] * X * X + 2 * q[1] * X * Y + 2 * q[2] * X * Z + 2 * q[3] * X +
    q[4] * Y * Y + 2 * q[5] * Y * Z + 2 * q[6] * Y + q[7] * Z * Z + 2 * q[8]
    * Z + q[9];
end; // vertex_error()
...
function calculate_error(id_v1, id_v2: integer; var p_result: TPoint3f;
  var vertices: TVs): TFloat; inline;
var
  q: TSymetricMatrix;
  border: integer;
...
begin
  ...
  if (error1 = result) then
    p_result := p1;
  if (error2 = result) then
    p_result := p2;
end; // calculate_error()
{$ELSE}

function calculate_error(id_v1, id_v2: integer; var p_result: TPoint3f;
  var vertices: TVs): TFloat; {$IFDEF FPC}inline; {$ENDIF}
var
  q: TSymetricMatrix;
  border: integer;
...
begin
  ...
  if (det <> 0) and (border = 0) then
  begin
    // q_delta is invertible
    p_result.X := -1 / det * (symMatDet(q, 1, 2, 3, 4, 5, 6, 5, 7, 8));
    // vx = A41/det(q_delta)
    p_result.Y := 1 / det * (symMatDet(q, 0, 2, 3, 1, 5, 6, 2, 7, 8));
    // vy = A42/det(q_delta)
    p_result.Z := -1 / det * (symMatDet(q, 0, 1, 3, 1, 4, 6, 2, 5, 8));
    // vz = A43/det(q_delta)
...
procedure update_mesh(iteration: integer; var triangles: TTs; var vertices: TVs;
  var refs: TRs; var nrefs: integer);
var
...
begin
  if (iteration > 0) then
  begin // compact triangles
    dst := 0;
    for i := 0 to high(triangles) do
    begin
      if (not triangles[i].deleted) then
      begin
  ...
  begin
    ...
    begin
      ...
      t^.n := n;
      for j := 0 to 2 do
        vertices[t^.v[j]].q := symMatAdd(vertices[t^.v[j]].q,
          symMat(n.X, n.Y, n.Z, -vDot(n, p3[0])));
    end; // for i: triangles
    for i := 0 to high(triangles) do
    begin
      // Calc Edge Error
  ...
  for i := 0 to high(triangles) do
    for j := 0 to 2 do
      vertices[triangles[i].v[j]].tcount := vertices[triangles[i].v[j]]
        .tcount + 1;
  tstart := 0;
  for i := 0 to high(vertices) do
  begin
  ...
  setlength(refs, length(triangles) * 6);
  // Pascal setlength() is expensive, so set larger than initially required to reduce re-allocation
  nrefs := length(refs);
  for i := 0 to high(triangles) do
  begin
    t := @triangles[i];
    ...
    begin
      ...
      begin
        ...
        begin
          ofs := 0;
          id := t^.v[k];
          while (ofs < length(vcount)) do
          begin
            if (vids[ofs] = id) then
              break;
            ofs := ofs + 1;
          end;
          if (ofs = length(vcount)) then
          begin
...
function flipped(p: TPoint3f; i1: integer; var v0: tvertex; var deleted: TBools;
  var triangles: TTs; var vertices: TVs; var refs: TRs): boolean;
{$IFDEF FPC}inline; {$ENDIF}
var
  k, s, id1, id2: integer;
  t: ^TTriangle;
...
procedure update_triangles(i0: integer; var v: tvertex; var deleted: TBools;
  var deleted_triangles: integer; var triangles: TTs; var vertices: TVs;
  var refs: TRs; var nrefs: integer); {$IFDEF FPC}inline; {$ENDIF}
const
  kBlockSz = 64000;
  // re-allocate memory in chunks - in future to accelerate perhaps use ReAllocMem instead of SetLength
var
  p: TPoint3f;
  k: integer;
  r: ^TRef;
...
begin
  ...
  begin
    ...
    ref := r^;
    // <- n.b. setlength() can change address of refs, so copy data to ref prior to resize!
...
begin
  ...
  for i := 0 to high(verts) do
  begin
    if (verts[i].X < mn.X) then
      mn.X := verts[i].X;
    if (verts[i].Y < mn.Y) then
      mn.Y := verts[i].Y;
    if (verts[i].Z < mn.Z) then
      mn.Z := verts[i].Z;
    if (verts[i].X > mx.X) then
      mx.X := verts[i].X;
    if (verts[i].Y > mx.Y) then
      mx.Y := verts[i].Y;
    if (verts[i].Z > mx.Z) then
      mx.Z := verts[i].Z;
  end;
...
var
  vertices: TVs;
  triangles: TTs;
  iteration, i, j, deleted_triangles, triangle_count, i0, i1, tstart,
    tcount: integer;
  ...
  nrefs: integer;
begin
  if (length(faces) < 5) or (length(verts) < 5) or (target_count < 4) or
    (target_count > length(faces)) then
    exit;
  scale := 1.0;
  if resize then
  ...
  begin
    ...
    begin
      t := @triangles[i];
      if (t^.err[3] > threshold) or (t^.deleted) or (t^.dirty) then
        continue;
      for j := 0 to 2 do
      begin
        if (t^.err[j] < threshold) then
        begin
          i0 := t^.v[j];
          v0 := @vertices[i0];
          i1 := t^.v[(j + 1) mod 3];
          ...
          if (flipped(p, i1, v0^, deleted0, triangles, vertices, refs)) then
            continue;
          if (flipped(p, i0, v1^, deleted1, triangles, vertices, refs)) then
            continue;
          // not flipped, so remove edge
          v0^.p := p;
          v0^.q := symMatAdd(v1^.q, v0^.q);
          tstart := nrefs; // length(refs);
          update_triangles(i0, v0^, deleted0, deleted_triangles, triangles,
            vertices, refs, nrefs);
          update_triangles(i0, v1^, deleted1, deleted_triangles, triangles,
            vertices, refs, nrefs);
          tcount := nrefs - tstart; // length(refs)-tstart;
          if (tcount <= v0^.tcount) then
          begin // save ram
            if (tcount > 0) then
              move(refs[tstart], refs[v0^.tstart], tcount * sizeof(TRef));
            // Move(src,dest,count);
          end
          else // append
            v0^.tstart := tstart;
